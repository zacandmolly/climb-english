...
async function status() {
  ...
  console.log(`OpenAI key: ${remoteKeyState()}`);
}
...
for line in lines:
    if line.startswith("OPENAI_API_KEY="):
        next_lines.append("OPENAI_API_KEY=" + key)
        found = True
    else:
        next_lines.append(line)
if not found:
    next_lines.insert(0, "OPENAI_API_KEY=" + key)
...
`;
  ...
  run('ssh', [sshHost, `python3 -c ${shellQuote(script)}`], { input: `${key}\n` });
  run('ssh', [sshHost, `launchctl kickstart -k gui/$(id -u)/${shellBare(serviceLabel)}`]);
  console.log('M1 API key updated and service restarted.');
  await status();
}
